...
/*
  Market A (ZenChain testnet)
  - Collateral: WZTC  (0x66D9541D1220488F5d569a8a43e80298Df145f4E) 18d
  - Debt/Supply: ZUSDC (0xF8aD5140d8B21D68366755DeF1fEFA2e2665060C) 6d
  - Oracle:      0x1b7e2509a34273d74c7F1767740E7dc2F63dcaAc (AdminPriceOracle)
  - Admin:       0x58E53eaDBed51C4AffDc06E7CCeD4bE1265d928F

  Functions:
    - supply/withdrawUnderlying (ZUSDC)
    - depositCollateral/withdrawCollateral (WZTC)
    - borrow/repay (ZUSDC)
    - liquidate
...
interface IPriceOracle {
    function getPriceE8(address token) external view returns (uint256); // USD, 1e8
}

contract MarketA_ZenFinal {
    ...
    // Decimals for math
    uint8  public constant COL_DEC = 18; // WZTC
    uint8  public constant DEBT_DEC = 6; // ZUSDC

    // Risk params (1e18)
    uint256 public constant LTV   = 65e16;   // 65%
    uint256 public constant LT    = 75e16;   // 75%
    uint256 public constant BONUS = 107e16;  // 1.07
    uint256 public constant CLOSE = 50e16;   // 50%
    uint256 public constant RES   = 10e16;   // Reserve factor 10%

    // Interest-rate model params (APR in 1e18)
    ...
    // Tokens and oracle (immutable)
    IERC20 public immutable collateralToken; // WZTC
    IERC20 public immutable debtToken;       // ZUSDC
    ...
    // Suppliers (simple share model, no ERC20 receipt)
    uint256 public totalShares;
    mapping(address => uint256) public shares; // supplier shares denominated in 1e18-rate

    // Borrowers/collateral
    mapping(address => uint256) public collateralBalance; // WZTC units
    ...
    constructor() {
        collateralToken = IERC20(WZTC);
        debtToken = IERC20(ZUSDC);
    ...
    function healthFactorOf(address u) public view returns (uint256) {
        ...
        uint256 colUSD = _toUSD(WZTC, COL_DEC, collateralBalance[u]);
        uint256 debtUSD = _toUSD(ZUSDC, DEBT_DEC, debt);
    ...
    function _borrowRatePerSecond(uint256 _cash, uint256 _borrows, uint256 _res) internal pure returns (uint256) {
        ...
        return apr / YEAR; // 1e18 per second
    }
    ...
    function withdrawUnderlying(uint256 amount) external nonReentrant {
        ...
        uint256 burnShares = (amount * 1e18 + rate - 1) / rate; // ceil
    ...
    function borrow(uint256 amount) external nonReentrant {
        ...
        // Check LTV
        uint256 colUSD = _toUSD(WZTC, COL_DEC, collateralBalance[msg.sender]);
        uint256 debtUSD = _toUSD(ZUSDC, DEBT_DEC, newOwed);
    ...
    function liquidate(address borrower, uint256 repayAmount) external nonReentrant {
        ...
        uint256 priceDebt = oracle.getPriceE8(ZUSDC);
        uint256 priceColl = oracle.getPriceE8(WZTC);
